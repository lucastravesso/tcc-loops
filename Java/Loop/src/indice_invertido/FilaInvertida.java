...
public class FilaInvertida {
    ...
    public static void main(String[] args) {
        ...
        List<List<Stack<Integer>>> pilhasPorValor = new ArrayList<List<Stack<Integer>>>();
        for (int j = 0; j < NUM_COLUNAS; j++) {
            ...
            for (int i = 0; i < NUM_LINHAS; i++) {
                if (mapa.containsKey(matriz[i][j])) {
                    ...
                    mapa.put(matriz[i][j], new ArrayList<Integer>(Arrays.asList(i)));
                }
            }
            List<Stack<Integer>> pilhas = new ArrayList<Stack<Integer>>();
